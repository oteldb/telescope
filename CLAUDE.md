...
| `internal/ui` | the bubbletea models. `app.go` is the root, `start.go` picks a place or a group and `saved.go` is what the config offers it, `logview.go` is the list, and what of a row's fields the width will take is spent there rather than in `logs`, so a resize changes it; `gutter.go` is the time column and the gaps down its left, `status.go` the bar over it — the counts, the toggles and what is being held back — `histogram.go` the volume panel above it, `entryview.go` one entry and `entrydoc.go` the rows its cursor walks, `origin.go` the column naming which stream a line came from, `clipboard.go` copies a row and `link.go` the command that reopens the list, `openview.go` is a list opened from the command line rather than from the start screen and `locate.go`/`open.go`/`stack.go` open what one points at, `page.go` asks for the lines before the first one when the reader reaches it, `complete.go` finishes what either filter prompt is typing — the list's and the start screen's — and `help.go` writes the filter language out, `theme.go` the palette, `rows.go` the row backgrounds, `logo.go` the banner. |
| `internal/mcp` | what `telescope mcp` serves an agent: a second consumer of `config`, `source` and `logs` beside `ui`, drawing nothing on a screen. `mcp.go` builds the server and registers each tool read-only, `places.go` says what the config declares and what each place holds, a trace store among them, since a store is a place and one list keeps the naming legible, `fields.go` asks a database what its lines are labeled with, `logs.go` reads a bounded window by walking `source.Page` back until it has enough matches, `summary.go` counts one instead of listing it, `render.go` writes both as the text a model reads and `placesrender.go` does the same for the listings, which would otherwise get their own facts serialized back as their text, `resolve.go` turns the name a tool was given back into a `source.Config`, and takes the `target` that finishes one the config declared without: a kubectl place is half a place until it is told which workload, and one that could be listed and never read is a place an agent can see and cannot use. `traces.go` reads one trace and `tracerender.go` draws it as offsets and nesting rather than as a gantt — a model reads numbers better than it reads a bar, and the one thing a bar showed that a number does not is what the offset column is for; `tracespan.go` answers one span with the attributes the tree leaves out, and `tracestore.go` says which store was meant, since a trace id names the request and not the place it was kept. `tracesearch.go` finds a trace to read and `searchrender.go` draws what it found — which counts appear follows from what the store reported, since Tempo counts the spans its query selected and Jaeger the spans the trace holds, and a column meaning whichever answered would be a number nobody can read; `tracefields.go` says what a store can be searched by, and keeps "the API has no such listing" apart from "there are none": Jaeger cannot enumerate tags, which is not the same as having none. `link.go` writes the command line that opens a view, which is how an agent points a person at what it found rather than describing it — the same type `ui` copies out, so the two ends say it the same way. A tool names a declared place and never a command: an agent that could name one could run anything the user can, over ssh. An answer's text is the lines and its structured content is the facts about them — never both, since a client that showed both would be paying twice. A client picks one half rather than merging them, so a half that cannot stand alone is not offered: `logs` and `trace` answer with their text only, because there the lines and the tree are the answer and the facts are counts about it; every other tool carries its whole answer in both. `mcp.go`'s `--log-file` writes the traffic, which is the only thing that tells a tool that answered nothing from a client that did not draw the answer. What was cut is always reported: a reader that cannot scroll has no other way to tell a quiet window from an unfinished one. |
...
| `internal/view` | one screen in a form that can be written down: a place, a filter, a window, or a trace. The written form is the command line that opens it, which runs where it is pasted, needs nothing registered with the desktop, and can be read by whoever is about to run it — it names a place that may reach production over ssh. It sits below everything that uses it: `cmd` binds flags to it, `ui` opens and copies one, `mcp` writes them out. |
